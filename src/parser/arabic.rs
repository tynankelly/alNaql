...
use std::path::Path;
use crate::config::parser::ParserConfig;
...
impl TextParser for ArabicParser {
    fn clean_text_with_mapping_absolute(&self, text: &str, absolute_position: usize) -> Result<TextMapping> {
        ...
        // Add this logging
        trace!("MAPPING_DEBUG: Cleaning text at absolute position {}, length {}", 
            absolute_position, text.len());
        trace!("MAPPING_DEBUG: Text sample: \"{}\"", 
        ...
        while i < chars.len() {
            ...
            if c == '<' {
                ...
                if looks_like_tag {
                    tag_depth += 1;
                    trace!("TAG DEPTH: Increased to {} at position {} (char '{}')", 
                           tag_depth, pos, c);
                }
            } else if c == '>' && tag_depth > 0 {
                tag_depth -= 1;
                trace!("TAG DEPTH: Decreased to {} at position {} (char '{}')", 
                       tag_depth, pos, c);
            }
            ...
            if tag_depth == 0 && self.is_countable_char(c) {
                ...
                if let Some(last_end) = last_arabic_end {
                    if pos > last_end && last_non_ignorable.is_some() && last_non_ignorable.unwrap() > last_end {
                        ...
                        cleaned.push(' ');
                        trace!("MAPPING_DEBUG: Added gap space, mapping ({}, {}) -> abs ({}, {})", 
                ...
                if cleaned.len() <= 10 || cleaned.len() % 50 == 0 {  // Limit logging to avoid overwhelming
                    trace!("MAPPING_DEBUG: Char '{}' at pos {} -> '{}' at index {} maps to abs ({}, {})", 
                ...
                trace!("SKIPPED ARABIC: '{}' at position {} due to being inside tag (depth: {})",
                       c, pos, tag_depth);
        ...
        trace!("POSITION MAPPING: Character mapping example:");
        for i in 0..std::cmp::min(10, char_map.len()) {
            ...
            trace!("POSITION MAPPING: Char '{}' at index {} maps to original position {:?}", 
                char, char_idx, abs_pos);
        }
    
        trace!("MAPPING_DEBUG: Finished mapping {} original bytes -> {} cleaned chars", 
        text.len(), cleaned.len());
        if !char_map.is_empty() {
            trace!("MAPPING_DEBUG: First mapped position: {:?}, Last: {:?}", 
