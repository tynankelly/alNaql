...
impl MetadataCache {
    ...
    pub fn with_thread_local_size(max_size: usize, ttl_seconds: u64, thread_local_size: usize) -> Self {
        trace!("Initializing metadata cache with max_size={}, ttl={}s, thread_local_size={}", 
    ...
    pub fn get(&self, key: u64) -> Option<Arc<NGramMetadata>> {
        ...
        LOCAL_METADATA_CACHE.with(|cache| {
            ...
            if cache_ref.max_size != self.thread_local_max_size {
                trace!("Updating thread-local cache size: {} -> {}", 
    ...
    fn evict_entries(&self, cache: &mut AHashMap<u64, (Arc<NGramMetadata>, Instant)>) {
        ...
        if evicted_count > 0 {
            ...
            trace!("Evicted {} entries from metadata cache", evicted_count);
        }
    }
    ...
    pub fn clear_thread_local(&self) {
        LOCAL_METADATA_CACHE.with(|cache| {
            ...
            trace!("Cleared thread-local metadata cache ({} entries)", len);
        });
    }
    ...
    pub fn print_stats(&self) {
        ...
        debug!("Metadata cache stats: {} hits, {} misses ({:.1}% hit rate), {} evictions, {} DB lookups ({:.0} avoided)",
        ...
        debug!("Shared metadata cache: {}/{} entries ({:.1}% full)", 
        ...
        debug!("Thread-local metadata cache: {:.1} average entries per thread, {} total evictions",
    ...
    pub fn clear_global_cache(&self) {
        ...
        self.stats.db_lookups.store(0, Ordering::Relaxed);
        
        debug!("Cleared global metadata cache: removed {} entries", size_before);
    }
    
    /// Clear both global and thread-local caches
    pub fn clear_all_caches(&self) {
        // Clear global cache
        self.clear_global_cache();
        
        // Clear current thread's local cache (can't clear other threads directly)
        self.clear_thread_local();
        
        debug!("Cleared all accessible caches");
    }
    
    /// Get current global cache size
    pub fn global_cache_size(&self) -> usize {
        self.shared_cache.read().unwrap().len()
    }
}
