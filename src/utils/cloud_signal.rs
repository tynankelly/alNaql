...
pub struct SpotTerminationHandler {
    ...
    /// Logger for events
    logger: Arc<Mutex<Option<Arc<Mutex<Logger>>>>>,
            ...
            while running.load(Ordering::SeqCst) {
                ...
                if is_ec2 && !termination_detected.load(Ordering::SeqCst) {
                    if check_spot_termination() {
                        ...
                        if let Ok(mut logger_guard_option) = logger.lock() {
                            // NOW create the logger since we have a termination
                            if logger_guard_option.is_none() {
                                let log_path = std::path::Path::new("logs").join(format!("spot_handler_{}.log", 
                                    chrono::Local::now().format("%Y%m%d_%H%M%S")));
                                
                                if let Ok(new_logger) = Logger::with_file(log_path) {
                                    *logger_guard_option = Some(Arc::new(Mutex::new(new_logger)));
                                }
                            }
                            
                            if let Some(ref logger_arc) = *logger_guard_option {
                                if let Ok(mut log) = logger_arc.lock() {
                                    let _ = log.log("⚠️ SPOT TERMINATION NOTICE DETECTED ⚠️");
                                    let _ = log.log("Starting graceful shutdown procedures");
                                }
                            }
                        }
                        ...
                        for path in &db_paths {
                            ...
                            if success {
                                ...
                                // Log emergency error
                                if let Ok(logger_option) = logger.lock() {
                                    if let Some(ref logger_arc) = *logger_option {
                                        if let Ok(mut log) = logger_arc.lock() {
                                            let message = format!(
                        ...
                        if let Ok(logger_option) = logger.lock() {
                            if let Some(ref logger_arc) = *logger_option {
                                if let Ok(mut log) = logger_arc.lock() {
                                    let _ = log.log("Graceful shutdown sequence completed");
                                }
                            }
                        }
                    }
...
fn try_lock_with_timeout<T>(mutex: &Mutex<T>, timeout: Duration) -> Option<std::sync::MutexGuard<'_, T>> {
