...
impl ProcessingManager {
    pub fn new(config: &AlNaqlConfig) -> Self {
        let mode = match config.execution.processing_mode.as_str() {
    ...
    where
        F: Fn(std::fs::DirEntry, &Path, &AlNaqlConfig, &MultiProgress) -> Fut + Send + Sync + Clone + 'static,
            ...
            ProcessingMode::Sequential => {
                for entry in files {
                    ...
                    if self.should_cleanup() {
                        ...
                        // Clear global caches
                        // Direct call - no error handling needed
                        METADATA_CACHE.clear_thread_local();
                        debug!("Cleared thread-local metadata cache");
                        
                        // Force database cleanup
                        if let Err(e) = crate::storage::perform_storage_maintenance() {
                            debug!("Storage maintenance during cleanup: {}", e);
                        }
                    ...
                    while let Ok(mem_info) = sys_info::mem_info() {
                        ...
                        if usage_percent > self.target_memory_percent {
                            ...
                            if usage_percent > 85.0 {
                                ...
                                if let Err(e) = crate::storage::perform_storage_maintenance() {
                                    warn!("Storage maintenance failed: {}", e);
