...
#[inline]
pub fn simd_jaccard(a: &[u8], b: &[u8]) -> f64 {
    // Add fast byte comparison
    if a == b {
        return 1.0;
    }
    
    // Decode to strings (we know they're valid UTF-8 from storage)
    let text_a = unsafe { std::str::from_utf8_unchecked(a) };
    let text_b = unsafe { std::str::from_utf8_unchecked(b) };
    
    // Your existing character extraction logic...
    ...
    let mut unique_chars = ahash::AHashSet::new();
    for &c in &chars_a {
    ...
    let mut char_to_index = ahash::AHashMap::new();
...
/// # Returns
/// A float value between 0.0 (completely different) and 1.0 (identical)
pub fn simd_cosine(a: &[u8], b: &[u8]) -> f64 {
    // Same pattern
    if a == b {
        return 1.0;
    }
