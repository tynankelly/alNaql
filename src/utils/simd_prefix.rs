...
/// Extract N characters from a UTF-8 byte slice
/// Uses SIMD for first 2 characters, scalar for the rest
/// 
/// # Arguments
/// * `bytes` - UTF-8 encoded byte slice  
/// * `depth` - Number of characters to extract (must be >= 2)
///
/// # Returns
/// * First character (if exists)
/// * Vector of remaining characters up to depth-1 (for use as key)
pub fn extract_n_chars(bytes: &[u8], depth: usize) -> (Option<char>, Vec<char>) {
    // Use existing SIMD for first 2
    let (first, second) = extract_first_two_chars(bytes);
    
    // If depth is 2 or we don't have enough chars, return what we have
    if depth == 2 || first.is_none() {
        return (first, vec![second.unwrap_or('\0')]);
    }
    
    // For depth > 2, continue with scalar extraction
    // Convert to string once (better than repeated conversions)
    let s = String::from_utf8_lossy(bytes);
    
    // Collect chars 2 through depth
    let mut key_chars = Vec::with_capacity(depth - 1);
    
    // Add second char from SIMD result
    if let Some(c) = second {
        key_chars.push(c);
    }
    
    // Add remaining chars up to depth
    for c in s.chars().skip(2).take(depth - 2) {
        key_chars.push(c);
    }
    
    // Pad with null bytes if string is too short
    while key_chars.len() < depth - 1 {
        key_chars.push('\0');
    }
